...
declare @Persona table (id int not null identity(1,1),
                        ...
                        nombre nvarchar(50),
                        fechaNac date)
declare @Cliente table (id nvarchar(20),
                        username nvarchar(50),
                        password nvarchar(50))
declare @Cuentas table (numCuenta nvarchar(30) not null,
                        saldo money,
                        fechaCreacion date,
...
begin
	...
	insert into @Persona(nombre,email,telefono,identificacion,fechaNac,idTipoIdent)
		select 
            ...
            tablaTemp.value('(docId/text())[1]','nvarchar(50)')as identificacion,
	...
	insert into @Benefic(idUsuario,idPersona,idParentesco,porcentajeBeneficio,activo,fechaDesactivacion)
		select
            tablaTemp.value('(numCuenta/text())[1]','nvarchar(50)')as idUsuario,
            tablaTemp.value('(docId/text())[1]','nvarchar(50)') as idPersona,
	...
	insert into Cliente(id,username,password) 
		...
		from	@Cliente c 
		inner join Persona p on p.identificacion=c.id
	insert into CuentaAhorro(numCuenta,saldo,idTipoCuenta,idCliente,fechaCreacion) 
		select	numCuenta,
				...
				fechaCreacion 
		from @Cuentas c
		inner join Persona p on p.identificacion=c.idCliente
