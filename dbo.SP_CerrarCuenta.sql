...
AS
BEGIN
	declare @saldoMinimoEDC money,@idCuenta int,@multaSM int,@saldoMinimoTC money,@idTipoCuenta int,@fechaInicio date,@saldoFinal money,@QRCH int,@Max_QRCH int,@multaExcesoRCH money,@montoCargosXSM money,@intereses float
	...
	select @intereses=t.tasaInteres*0.01, @saldoMinimoTC=t.saldoMinimo,@multaSM=(-1*t.multaSaldoMinimo),@Max_QRCH=t.maxRetiroEnCajeros, @montoCargosXSM=-t.montoMensual from TipoCuenta t where t.id=@idTipoCuenta
	...
	select @multaExcesoRCH=saldo*@intereses from CuentaAhorro where id=@idCuenta
	exec dbo.SP_procesarMovimiento @multaExcesoRCH,@idCuenta,@fechaCierre,'Intereses ganados',7,@idEstadoDeCuenta
